            ...
            @if($user->employee)
                <div class="mt-4 flex flex-col sm:flex-row items-center justify-center gap-4">
                    <div class="inline-flex items-center bg-white dark:bg-slate-800 px-4 py-2 rounded-full">
                        ...
                        <span class="text-slate-900 dark:text-slate-100 font-medium">{{ $user->employee->first_name }} {{ $user->employee->last_name }}</span>
                        @if($user->employee->department)
                            <span class="text-slate-500 dark:text-slate-300 ml-2">• {{ $user->employee->department->name }}</span>
                        @endif
                    </div>
                    <div class="inline-flex items-center bg-white dark:bg-slate-800 px-4 py-2 rounded-full">
                        <i class="fas fa-id-badge text-green-600 dark:text-emerald-300 mr-2"></i>
                        <span class="text-slate-900 dark:text-slate-100 font-medium">Employee ID: {{ $user->employee->employee_id }}</span>
                    </div>
                    @if($todayAttendance)
                        <div class="inline-flex items-center bg-white dark:bg-slate-800 px-4 py-2 rounded-full">
                            <i class="fas fa-clock text-blue-600 dark:text-blue-300 mr-2"></i>
                            <span class="text-slate-900 dark:text-slate-100 font-medium">Status: {{ ucfirst($todayAttendance->status) }}</span>
                        </div>
                    @endif
                </div>
            @endif
